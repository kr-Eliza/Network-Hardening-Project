...
SSH_OK_OUTPUT=$(ssh -i "${SSH_KEY}" -o StrictHostKeyChecking=accept-new \
    -o ConnectTimeout=5 -o BatchMode=yes "${SSH_USER}@${SSH_TARGET}" whoami 2>&1)
SSH_OK_STATUS=$?
echo "$SSH_OK_OUTPUT"
...
SSH_FAIL_OUTPUT=$(ssh -o StrictHostKeyChecking=accept-new \
    -o PubkeyAuthentication=no -o PreferredAuthentications=password \
    -o BatchMode=yes -o ConnectTimeout=5 "${SSH_USER}@${SSH_TARGET}" whoami 2>&1)
SSH_FAIL_STATUS=$?
...
IPSEC_OUTPUT=$(ssh -o StrictHostKeyChecking=accept-new -o ConnectTimeout=5 \
    student@"${GW_IPSEC_HOST}" "sudo ipsec statusall" 2>&1)
IPSEC_STATUS=$?
echo "$IPSEC_OUTPUT"
